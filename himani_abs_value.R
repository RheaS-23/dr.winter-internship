...
HCD.data <- Read10X_h5("C:/Users/Himani/Downloads/matrix.h5")
HCD <- CreateSeuratObject(counts = HCD.data)

HCP.data <- Read10X_h5("C:/Users/Himani/Downloads/filtered_feature_bc_matrix.h5")
HCP <- CreateSeuratObject(counts = HCP.data)

IWD.data <- Read10X_h5("C:/Users/Himani/Downloads/filtered_feature_bc_matrix (1).h5")
IWD <- CreateSeuratObject(counts = IWD.data)

IWP.data <- Read10X_h5("C:/Users/Himani/Downloads/filtered_feature_bc_matrix (2).h5")
IWP <- CreateSeuratObject(counts = IWP.data)

P0D.data <- Read10X_h5("C:/Users/Himani/Downloads/filtered_feature_bc_matrix (3).h5")
P0D <- CreateSeuratObject(counts = P0D.data)

P0P.data <- Read10X_h5("C:/Users/Himani/Downloads/filtered_feature_bc_matrix (4).h5")
P0P <- CreateSeuratObject(counts = P0P.data)

# visualizing the object before the QC strategy using a Vln Plot

#first calculating the percent mt
...
#getting vln plot for before QC 
merged <- merge(HCD, y = c(HCP, IWD, IWP, P0D, P0P), add.cell.ids = c("HCD", "HCP", "IWD","IWP", "P0D", "P0P"))
VlnPlot(merged, features = c("nCount_RNA", "nFeature_RNA", "percent.mt"), pt.size = 0, group.by = "orig.ident")


#using absolute value QC strategy (identifying & removing low-quality cells by cut-offs)
